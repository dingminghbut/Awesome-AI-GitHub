...
- Optimizing product titles and keyword-rich listing titles: start with [Listing Title Generation](${REPOSITORY.pagesUrl}title-generation.html).
- Writing Amazon five bullets and selling-point copy: start with [Five-Bullet Copy Generation](${REPOSITORY.pagesUrl}bullet-points.html).
- Creating marketplace main images and conversion-focused hero visuals: start with [Main Image Generation](${REPOSITORY.pagesUrl}main-image.html).
- Creating secondary images, lifestyle scenes, infographics, and carousel assets: start with [Secondary Image & Gallery Generation](${REPOSITORY.pagesUrl}secondary-images.html).
- Building Amazon A+ content, enhanced brand modules, and rich detail pages: start with [A+ Content Generation](${REPOSITORY.pagesUrl}a-plus.html).
- Generating product FAQs, buyer Q&A, objection handling, and support macros: start with [FAQ Generation](${REPOSITORY.pagesUrl}faq-generation.html).
...
- 做商品标题、关键词标题或标题优化：先看[标题生成](${REPOSITORY.pagesUrl}title-generation.html)。
- 做 Amazon 五点、商品卖点或利益点文案：先看[五点描述生成](${REPOSITORY.pagesUrl}bullet-points.html)。
- 做平台主图、首图、白底图或转化首图：先看[主图生成](${REPOSITORY.pagesUrl}main-image.html)。
- 做附图、场景图、信息图、对比图或轮播图：先看[附图与场景图生成](${REPOSITORY.pagesUrl}secondary-images.html)。
- 做 Amazon A+、品牌增强内容或富内容详情页：先看[A+ 页面生成](${REPOSITORY.pagesUrl}a-plus.html)。
- 做商品 FAQ、买家问答、异议处理或客服话术：先看[FAQ 生成](${REPOSITORY.pagesUrl}faq-generation.html)。
...
- 使用 GitHub Trending、Topics、Explore 作为官方发现种子，再通过仓库 API 拉取标准仓库数据，并经过同一套电商相关性过滤。
...
function categoryAudience(slug: string, language: "en" | "zh"): string {
  const text = {
    en: {
      ...
      "title-generation": "Useful for operators and builders generating keyword-rich marketplace titles, SEO titles, and title optimization workflows.",
      "bullet-points": "Useful for Amazon sellers and listing teams writing five bullets, selling points, feature-benefit copy, and concise listing copy.",
      "main-image": "Useful for teams producing marketplace main images, product hero visuals, white-background shots, and conversion-focused primary images.",
      "secondary-images": "Useful for teams generating product gallery assets, lifestyle scenes, comparison images, infographics, detail images, and carousel creatives.",
      "a-plus": "Useful for Amazon and marketplace teams building A+ content, enhanced brand content, brand story modules, and rich product detail pages.",
      "faq-generation": "Useful for teams generating product FAQs, buyer Q&A, objection handling, pre-sales answers, support macros, and listing questions.",
    ...
    zh: {
      ...
      "title-generation": "适合生成关键词商品标题、平台标题、SEO 标题和标题优化工作流的运营者与开发者。",
      "bullet-points": "适合 Amazon 卖家和 Listing 团队生成五点描述、商品卖点、功能利益点和精炼列表文案。",
      "main-image": "适合制作平台主图、商品首图、白底图和转化导向主视觉的团队。",
      "secondary-images": "适合生成商品附图、场景图、对比图、信息图、详情图和轮播素材的团队。",
      "a-plus": "适合 Amazon 和跨境平台团队制作 A+ 内容、品牌增强内容、品牌故事模块和富内容详情页。",
      "faq-generation": "适合生成商品 FAQ、买家问答、异议处理、售前回答、客服话术和详情页问题的团队。",
